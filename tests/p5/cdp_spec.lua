describe("cdp", function()
	...
	before_each(function()
		...
		cdp.tab_data = {
			...
			perf = { fps = {}, heap = 0, heap_total = 0, nodes = 0, listeners = 0, recording = true },
			info = { symbols = {}, canvas_state = "" },
		}
		server.server_job = 1
		server.port = 9999
		core.find_chrome = function()
	...
	describe("_decode_screenshot", function()
		it("decodes valid base64", function()
			local raw = cdp._decode_screenshot(vim.base64.encode("iVBOR"))
			assert.equals("iVBOR", raw)
		end)

		it("returns nil for invalid input", function()
			assert.is_nil(cdp._decode_screenshot(""))
			assert.is_nil(cdp._decode_screenshot("!!not-base64!!"))
			assert.is_nil(cdp._decode_screenshot(nil))
		end)
	end)

	describe("_format_canvas_state", function()
		it("formats a p5 runtime snapshot", function()
			local s = cdp._format_canvas_state({ mode = "global", w = 800, h = 600, frameRate = 60, frameCount = 42 })
			assert.equals("p5 global | canvas 800x600 | ~60 fps | frame 42", s)
		end)

		it("handles missing canvas", function()
			local s = cdp._format_canvas_state({ mode = "none", w = nil, h = nil, frameRate = nil, frameCount = nil })
			assert.equals("p5 none", s)
		end)

		it("returns nil for non-table", function()
			assert.is_nil(cdp._format_canvas_state(nil))
			assert.is_nil(cdp._format_canvas_state("x"))
		end)
	end)
