...
G.mkgist = function(description)
	...
	local files = G.includes(dir, config)
	...
	local function proceed(desc, proj_dir, proj_config)
		...
		local gist_temp_dir = vim.fn.stdpath("cache") or (vim.uv.os_tmpdir() or "/tmp") .. "/p5_gist"
	...
	if description and description ~= "" then
		proceed(description, dir, config)
	else
...
G.sync = function(gist_id)
	...
	local files_to_update = G.includes(dir, config)
...
G.list_gists = function()
	if not chkgh() then
		core.notify("GitHub CLI (gh) not found", "info")
		return
	end
